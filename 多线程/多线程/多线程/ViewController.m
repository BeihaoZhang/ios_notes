...
/*
 ...
 Q：什么情况下使用自旋锁比较划算？
 A：
 首先，OSSpinLock是个自旋锁，但会出现优先级反转，所以不推荐使用。而它的替代方案os_unfair_lock从调试过程来看，是个互斥锁。所以在iOS中不用纠结这个问题。
 ...
 1）预计线程等待锁的时间较长
 2）单核处理器
 3）临界区有IO操作（如文件读取与写入，因为IO操作比较占用CPU资源，如果用自旋锁，它在自旋的过程中也要占用CPU资源，会导致CPU资源更加紧张）
 4）临界区代码复杂或者循环量大
 5）临界区竞争非常激烈
